import React from 'react'

function SSBody() {
    return (
        <div className="bodyDiv">
        <img src="legal_image.jpg" alt="Italian Trulli" className="landingImage"/>
            <div className="intraMainPicText">
                <p className="whiteTextBackground"> 
                    <br/>
                    SocialSecuritySocialSecuritySocialSecuritySocialSecuritySocialSecuritySocialSecuritySocialSecurity
                     orem ipsum, or lipsum as it is sometimes known, is dummy text used in laying out print, 
                    graphic or web designs. The passage is attributed to an unknown typesetter in the 15th century 
                    who is thought to have scrambled 
                </p>
                <p className="whiteTextBackground"> 
                    <br/>
                    Lorem ipsum, or lipsum as it is sometimes known, is dummy text used in laying out print, 
                    graphic or web designs. The passage is attributed to an unknown typesetter in the 15th century 
                    who is thought to have scrambled 
                </p>
                <p className="whiteTextBackground"> 
                    <br/>
                    Lorem ipsum, or lipsum as it is sometimes known, is dummy text used in laying out print, 
                    graphic or web designs. The passage is attributed to an unknown typesetter in the 15th century 
                    who is thought to have scrambled 
                </p>

 
        </div>


  </div>
    )
}

export default SSBody